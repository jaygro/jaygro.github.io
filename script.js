...
let bedCount = 1;
let selectedLanguage = 'en'; // Assuming this exists from your language setup; adjust if needed
...
function updateLanguage() {
  console.log('Starting updateLanguage...');
  try {
    selectedLanguage = document.getElementById('language').value || 'en'; // Adjust ID if yours differs
    console.log('Language selected:', selectedLanguage);
    populateCropDropdowns(); // Refresh crop dropdowns with new language
  } catch (error) {
...
function populateCropDropdowns() {
  ...
  try {
    ...
    const defaultOption = '<option value="">Cultivo:</option>';
    const options = defaultOption + crops.map(crop => {
      const displayName = cropTranslations['es'][crop] || crop; // Hardcoded 'es'
      console.log(`Looking up ${crop} in es: ${cropTranslations['es'][crop]}`);
...
>>>>>>> fb54fe31b48f89adfe41b20ce21ee3f377a8cfde
